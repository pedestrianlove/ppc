    ...
    TestCaseInstance generate(ppc::random &rng) const override {
        ...
        for (int y = 0; y < Ny; ++y) {
            ...
            for (int x = 0; x < Nx; ++x) {
                ...
                if (inside) {
                    for (int c = 0; c < 3; ++c) {
                        bitmap[pixel_base + c] = (c == channel) ? color_inner : 0;
                    }
                } else if (inside_gradient) {
                    const float val = float(y2 - y) * fact * (color_inner - color_outer) + color_outer;
                    for (int c = 0; c < 3; ++c) {
                        bitmap[pixel_base + c] = (c == channel) ? val : 0;
                    }
                } else {
                    for (int c = 0; c < 3; ++c) {
                        bitmap[pixel_base + c] = (c == channel) ? color_outer : 0;
                    }
                }
            }
        }

        Result e = segment_gradient(x0, x1, y0, y1, y2, channel, bitmap.data());
        return {Ny, Nx, std::move(bitmap), e};
    }

    Result segment_gradient(int x0, int x1, int y0, int y1, int y2, int channel, const float *data) const {
        // We know all the boundaries, except inside the gradient
        pfloat color_outer;
        pfloat color_inner = data[3 * (Nx * y0 + x0) + channel];

        if (x0 > 0 || y0 > 0) {
            const pfloat gr_color = data[channel];
            color_outer = gr_color;
        } else if (x1 < Nx || y2 < Ny) {
            const pfloat gr_color = data[3 * (Nx * Ny) - 3 + channel];
            color_outer = gr_color;
        } else {
            throw;
        ...
        for (int yend = y1; yend <= y2; ++yend) {
            ...
            for (int y = y1; y < yend; ++y) {
                ...
                const pfloat gr_color = data[pixel_base + channel];
            ...
            if (sqerror < min_sqerror) {
                ...
                e.outer[channel] = color_outside;
                e.inner[channel] = color_inside;
                e.y0 = y0;
                e.y1 = yend;
