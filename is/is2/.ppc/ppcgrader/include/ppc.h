...
class random {
  public:
    ...
    float get_float() {
        ...
        static_assert(std::numeric_limits<float>::digits == 24, "Floating-point numbers have exactly 24 bits in the mantissa");
    ...
    double get_double() {
        ...
        static_assert(std::numeric_limits<double>::digits == 53, "Double-precision floating-point numbers have exactly 53 bits in the mantissa");
...
inline void setup_cuda_device() {
    ...
    if (err != cudaSuccess) {
        std::cerr << "Failed to set up CUDA device:" << cudaGetErrorString(err) << std::endl;
