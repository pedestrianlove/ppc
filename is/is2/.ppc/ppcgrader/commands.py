...
class TestAsanCommand(SmallTestCommandBase):
    start_message = 'Running tests with AddressSanitizer'
    name = 'test-asan'
...
class AssemblyCommandBase(Command):
    ...
    def exec(self, compiler: Optional[Compiler], reporter: Reporter,
        ...
        if len(assembly.encode('utf-8')) > MAX_ASSEMBLY_OUTPUT:
            assembly = "Generated assembly was too long and was not stored"
    ...
    def run_demo(self, reporter: Reporter, args: List[str],
        ...
        try:
            ...
            reporter.log(f"Demo returned with error code {error.returncode}",
                         'error')
            reporter.log("Output was:", 'heading')
...
def no_tests_error(orig_tests: List[str], dirs: List[str]):
    if orig_tests:
        ...
        sys.exit(
            ...
            +
            'Please give a path to a test file or a glob that expands to tests.'
        )
    else:
        assert len(dirs) > 0
        if len(dirs) == 1:
            deleted_target = f'the {dirs[0]} folder or its contents'
        else:
            dir_names = ', '.join(dirs[:-1]) + ' or ' + dirs[-1]
            deleted_target = f'the {dir_names} folders or their contents'

        sys.exit(
            f'Couldn\'t find default tests. Have you accidentally deleted {deleted_target}?'
        )
