...
class TerminalReporter(Reporter):
    def check_timeout(self, output: RunnerOutput):
        grader = get_grader_domain("cli")

        if not output.is_success():
            self.log_sep()
            if output.is_timed_out():
                self.log(grader.gettext("reporter.timeout.timed_out", ))
                self.log(
                    grader.gettext(
                        "reporter.timeout.expected_duration",
                        timeout=output.timeout,
                    ))
                self.log(grader.gettext("reporter.timeout.hint_override", ))
            else:
                self.log(
                    grader.gettext("reporter.timeout.crashed_unexpectedly", ))
            self.log_sep()
    ...
    def log_nvprof_output(self: 'TerminalReporter',
        ...
        if output.nvprof:
            if output.nvprof.get('gpu_trace') is not None:
                ...
                if t:
                    self.log_sep()
                    self.log(grader.gettext("reporter.nvprof.trace_heading", ))
            ...
            self.log_sep()
            self.log(grader.gettext("reporter.nvprof.failed_parse", ))
            self.log(output.nvprof_raw, 'output')
            self.log_sep()
        else:
            self.log_sep()
            self.log(grader.gettext("reporter.nvprof.no_output", ))
            self.log_sep()
    ...
    def log_asan_output(self: 'TerminalReporter', output: AsanRunnerOutput):
        ...
        if output.asanoutput:
            self.log_sep()
            self.log(grader.gettext("reporter.asan.heading", ))
        ...
        if platform.system() == 'Linux' and not output.run_successful:
            ...
            if "AddressSanitizer:DEADLYSIGNAL\nAddressSanitizer:DEADLYSIGNAL" in output.stderr:
                self.log_sep()
                self.log(grader.gettext("reporter.asan.aslr_bug", ))
                self.log_sep()
    ...
    def log_memcheck_output(self: 'TerminalReporter',
        ...
        if output.memcheckoutput:
            self.log_sep()
            self.log(grader.gettext("reporter.memcheck.heading", ))
    ...
    def print_test_case(self, group: 'RunGroupBase', test: str,
        ...
        if output.stdout:
            self.log_sep()
            self.log(grader.gettext("reporter.streams.stdout_heading", ))
            self.log(output.stdout, 'output')
            self.log_sep()

        if output.stderr:
            self.log_sep()
            self.log(grader.gettext("reporter.streams.stderr_heading", ))
    ...
    class AnalysisGroup(Reporter.AnalysisGroup):
        ...
        def analyze(self, output, success=True):
            ...
            self.reporter.log(
                grader.gettext(
                    "reporter.analysis.output_heading",
                    name=self.name,
                ),
                'heading',
            )
    ...
    class CompilationProxy(Reporter.CompilationProxy):
        ...
        def compile(self, *args, **kwargs) -> 'CompilerOutput':
            ...
            self.reporter.log(grader.gettext("reporter.compile.start", ))
            result = self.compiler.compile(*args, **kwargs)
            if result.stdout:
                self.reporter.log_sep()
                self.reporter.log(
                    grader.gettext("reporter.compile.stdout_heading", ))
                self.reporter.log(result.stdout, 'output')
                self.reporter.log_sep()
            if result.stderr:
                self.reporter.log_sep()
                self.reporter.log(
                    grader.gettext("reporter.compile.stderr_heading", ))
                self.reporter.log(result.stderr, 'output')
                self.reporter.log_sep()
            if result.is_success():
                self.reporter.log(grader.gettext("reporter.compile.success", ))
            else:
                self.reporter.log(
                    grader.gettext("reporter.compile.failure", ),
                    'error',
                )
                self.reporter.log_sep()
        ...
        def print_errors(self, stderr: str):
            ...
            for error in errors:
                if error['type'] == "Wvla":
                    ...
                    self.reporter.log_sep()
                    self.reporter.log(
                        grader.gettext("reporter.compiler.wvla_error", ),
                        'output',
                    )
                    self.reporter.log_sep()
                elif error['type'] == "omp":
                    ...
                    if self.reporter.config.openmp:
                        self.reporter.log(
                            grader.gettext(
                                "reporter.compiler.omp_true_error", ),
                            'output',
                        )
                    else:
                        self.reporter.log(
                            grader.gettext(
                                "reporter.compiler.omp_false_error", ),
                            'output',
                        )
                    self.reporter.log_sep()
