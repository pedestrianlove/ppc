...
def clang_compiler(name):
    ...
    if compiler is None:
        raise argparse.ArgumentTypeError(
            f"Couldn't automatically find a clang compiler")
...
def gcc_compiler(name):
    ...
    if compiler is None:
        raise argparse.ArgumentTypeError(
            f"Couldn't automatically find a gcc compiler")
    elif not compiler.is_valid():
        raise argparse.ArgumentTypeError(
...
def nvcc_compiler(name):
    ...
    if compiler is None:
        raise argparse.ArgumentTypeError(
            f"Couldn't automatically find an nvcc compiler")
    elif not compiler.is_valid():
        raise argparse.ArgumentTypeError(
    ...
    class HelpAction(argparse.Action):
        ...
        def __call__(self, parser: argparse.ArgumentParser,
            ...
            copy.add_argument(
                ...
                metavar='tests',
                help=
                'tests to run; can be a single file, a list, or a glob of test files: tests/<filename1>.txt tests/<filename2>.txt or \'tests/*\''
            )
...
def prepare_parser(config: Config, remote: bool = False):
    ...
    compiler_group.add_argument(
        ...
        const='',
        help=
        'use the given clang compiler; if left empty, try to find a suitable compiler',
        type=compiler('clang'))
    compiler_group.add_argument(
        '--gcc',
        dest='compiler',
        ...
        const='',
        help=
        'use the given gcc compiler; if left empty, try to find a suitable compiler',
        type=compiler('gcc'))
    compiler_group.add_argument(
        '--nvcc',
        dest='compiler',
        ...
        const='',
        help=
        'use the given nvcc compiler; if left empty, try to find a suitable compiler',
        type=compiler('nvcc'))
...
def parse_args(raw_args, config: Config, remote: bool = False):
    ...
    for test in tests:
        ...
        if test in __commands:
            print(
                f'./grading: warning: The grader takes only one command. Argument {test} will be parsed as a test file',
                file=sys.stderr)
